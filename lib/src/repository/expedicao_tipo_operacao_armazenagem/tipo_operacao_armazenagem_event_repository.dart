...
class TipoOperacaoArmazenagemEventRepository implements EventContract {
  static TipoOperacaoArmazenagemEventRepository? _instancia;
  ...
  TipoOperacaoArmazenagemEventRepository._() {
    _onInsert();
    _onUpdate();
    _onDelete();
  }

  static TipoOperacaoArmazenagemEventRepository get instancia {
    _instancia ??= TipoOperacaoArmazenagemEventRepository._();
    return _instancia!;
  }
  ...
  void _onInsert() {
    const event = 'tipo.operacao.armazenagem.insert.listen';
  ...
  void _onUpdate() {
    const event = 'tipo.operacao.armazenagem.update.listen';
    _appSocket.socket.on(event, (data) {
      _listener
  ...
  void _onDelete() {
    const event = 'tipo.operacao.armazenagem.delete.listen';
    _appSocket.socket.on(event, (data) {
      _listener
