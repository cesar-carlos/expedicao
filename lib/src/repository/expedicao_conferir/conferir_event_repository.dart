...
class ConferirEventRepository implements EventContract {
  static ConferirEventRepository? _instancia;
  ...
  ConferirEventRepository._() {
    _onInsert();
    _onUpdate();
    _onDelete();
  }

  static ConferirEventRepository get instancia {
    _instancia ??= ConferirEventRepository._();
    return _instancia!;
  }
  ...
  void _onInsert() {
    const event = 'conferir.insert.listen';
  ...
  void _onUpdate() {
    const event = 'conferir.update.listen';
    _appSocket.socket.on(event, (data) {
      _listener
  ...
  void _onDelete() {
    const event = 'conferir.delete.listen';
    _appSocket.socket.on(event, (data) {
      _listener
