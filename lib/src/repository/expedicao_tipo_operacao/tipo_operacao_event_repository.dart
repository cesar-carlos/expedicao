...
class TipoOperacaoEventRepository implements EventContract {
  static TipoOperacaoEventRepository? _instancia;
  ...
  TipoOperacaoEventRepository._() {
    _onInsert();
    _onUpdate();
    _onDelete();
  }

  static TipoOperacaoEventRepository get instancia {
    _instancia ??= TipoOperacaoEventRepository._();
    return _instancia!;
  }
  ...
  void _onInsert() {
    const event = 'tipo.operacao.expedicao.insert.listen';
  ...
  void _onUpdate() {
    const event = 'tipo.operacao.expedicao.update.listen';
    _appSocket.socket.on(event, (data) {
      _listener
  ...
  void _onDelete() {
    const event = 'tipo.operacao.expedicao.delete.listen';
    _appSocket.socket.on(event, (data) {
      _listener
