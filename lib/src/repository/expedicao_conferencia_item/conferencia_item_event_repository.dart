...
class ConferenciaItemEventRepository implements EventContract {
  static ConferenciaItemEventRepository? _instancia;
  ...
  ConferenciaItemEventRepository._() {
    _onInsert();
    _onUpdate();
    _onDelete();
  }

  static ConferenciaItemEventRepository get instancia {
    _instancia ??= ConferenciaItemEventRepository._();
    return _instancia!;
  }
  ...
  void _onInsert() {
    const event = 'conferencia.item.insert.listen';
  ...
  void _onUpdate() {
    const event = 'conferencia.item.update.listen';
    _appSocket.socket.on(event, (data) {
      _listener
  ...
  void _onDelete() {
    const event = 'conferencia.item.delete.listen';
    _appSocket.socket.on(event, (data) {
      _listener
