...
class CarrinhoPercursoAgrupamentoEventRepository implements EventContract {
  static CarrinhoPercursoAgrupamentoEventRepository? _instancia;
  ...
  CarrinhoPercursoAgrupamentoEventRepository._() {
    _onInsert();
    _onUpdate();
    _onDelete();
  }

  static CarrinhoPercursoAgrupamentoEventRepository get instancia {
    _instancia ??= CarrinhoPercursoAgrupamentoEventRepository._();
    return _instancia!;
  }
  ...
  void _onInsert() {
    const event = 'carrinho.percurso.agrupamento.insert.listen';
  ...
  void _onUpdate() {
    const event = 'carrinho.percurso.agrupamento.update.listen';
    _appSocket.socket.on(event, (data) {
      _listener
  ...
  void _onDelete() {
    const event = 'carrinho.percurso.agrupamento.delete.listen';
    _appSocket.socket.on(event, (data) {
      _listener
